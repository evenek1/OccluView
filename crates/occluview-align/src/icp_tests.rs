...
fn where_the_file_puts_its_zero_does_not_change_the_refine() {
    ...
    // A micron, not an epsilon: vertices are `f32`, so quoting them from two
    // metres away costs about 1e-4 mm of representable precision before the
    // solver sees them. Anything at that scale is the storage format, not the fit;
    // a real difference in what the guard or the solver did would be orders of
    // magnitude larger.
