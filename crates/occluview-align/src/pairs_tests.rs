...
fn turning_a_scan_over_where_it_stands_is_not_a_runaway() {
    // The case the guard used to refuse, with the numbers measured off a real
    // arch on disk: bite_research/dqjob/upper.stl is 88.23 mm across its
    // bounding-box diagonal and its vertices sit 70.80 mm from the file's own
    // zero. The partner scan is the same arch 2.3 mm away, stored with the
...
fn where_the_file_puts_its_zero_does_not_change_the_verdict() {
    ...
    //
    // The distances are the ones the format actually produces: on top of the
    // geometry, the 70.80 mm measured off bite_research/dqjob/upper.stl, and
    ...
    for zero in [0.0, 70.80, 400.0, 2000.0] {
