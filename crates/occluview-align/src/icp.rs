...
pub fn refine(
    ...
    // turning a mesh in place swings that column by twice the distance from
    // the file's zero to the geometry, which on a real arch is up to 71 mm of
    // pure bookkeeping. Reading it as displacement refused fits that had not
