...
settings-save-format-source-hint = A scan opened as STL is saved as STL, a scan opened as PLY as PLY, and a scan opened from a format with no writer is saved in the fallback format named below.
...
settings-save-format-scene-note = A whole scene saved as one file always uses { $format }.
