...
/// and every model on screen came out half again too wide. Fullscreen only,
/// because that is where the width first passes 2560.
#[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
fn fitted_render_extent_px(width_px: f32, height_px: f32) -> [u16; 2] {
    ...
    // Growing wins: a target with an axis under the floor is the degenerate
    // case, and it can only ask for growth when nothing is over the ceiling.
...
mod tests {
    ...
    fn a_fullscreen_4k_viewport_keeps_its_shape() {
        // The reported bug: models distorted in fullscreen on a 4K display and
        // nowhere else, because fullscreen is where the width first passes the
