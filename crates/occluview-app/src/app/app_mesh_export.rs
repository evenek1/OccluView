    ...
    pub(super) fn save_layer_export_dialog(
        ...
        match write_layer_export_to_path(scene, request, &path) {
            Ok(report) => {
                ...
                let warnings = mesh_export_warning_summary(&report.warnings, &self.locale);
                ...
                self.status_message = Some(self.locale.tr_with(
                    status_key,
                    &[
                        ("name", name.as_str()),
            ...
            Err(error) => {
                ...
                self.app_error = Some(AppErrorDialog {
                    title: self.locale.tr("mesh-export-failed-title"),
                    summary,
                    details: format!(
                        "Layer export failed\n\nPath:\n{}\n\nError:\n{error:#}",
                        path.display()
